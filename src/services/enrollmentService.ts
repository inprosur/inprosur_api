...
export const getEnrollmentById = async (id: number): Promise<Enrollment | null> => {
    const result = await db.execute("SELECT * FROM enrollments WHERE id = ?", [id]);
    const rows = Array.isArray(result) ? result[0] : result.rows;
    return rows.length === 1 ? rows[0] as Enrollment : null;
};

export const createEnrollment = async (enrollment: Omit<Enrollment, 'id' | 'enrollmentDate' | 'paymentDate'>): Promise<Enrollment> => {
