...
export const getAllCommissions = async (): Promise<InstructorCommission[]> => {
    const result = await db.execute("SELECT * FROM InstructorCommissions");
    const rows = Array.isArray(result) ? result[0] : result.rows;
    return rows as InstructorCommission[];
};

export const getCommissionById = async (id: number): Promise<InstructorCommission | null> => {
    const result = await db.execute("SELECT * FROM InstructorCommissions WHERE id = ?", [id]);
    const rows = Array.isArray(result) ? result[0] : result.rows;
    return rows.length === 1 ? rows[0] as InstructorCommission : null;
};

export const createCommission = async (commission: Omit<InstructorCommission, 'id' | 'paymentDate'>): Promise<InstructorCommission> => {
