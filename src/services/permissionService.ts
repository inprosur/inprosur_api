...
export const createPermission = async (permission: Permission): Promise<Permission> => {
    const result = await db.execute('INSERT INTO Permissions (name, description) VALUES (?, ?)', [permission.name, permission.description]);
    const rows = Array.isArray(result) ? result[0] : result.rows;
    const newPermission = { ...permission, id: rows.insertId };
