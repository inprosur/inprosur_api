...
export const getAllAccessLogs = async (): Promise<AccessLog[]> => {
    const result = await db.execute("SELECT * FROM AccessLog");
    const rows = Array.isArray(result) ? result[0] : result.rows;
    return rows as AccessLog[];
};

export const getAccessLogById = async (id: number): Promise<AccessLog | null> => {
    const result = await db.execute("SELECT * FROM AccessLog WHERE id = ?", [id]);
    const rows = Array.isArray(result) ? result[0] : result.rows;
    return rows.length === 1 ? rows[0] as AccessLog : null;
};

export const createAccessLog = async (accessLog: AccessLog): Promise<AccessLog> => {
